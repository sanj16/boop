...
var require_iter = __commonJS({
  "node_modules/obliterator/iter.js"(exports2, module2) {
...
var require_take = __commonJS({
  "node_modules/obliterator/take.js"(exports2, module2) {
    var iter = require_iter();
    module2.exports = function take(iterable, n2) {
      var l2 = arguments.length > 1 ? n2 : Infinity, array = l2 !== Infinity ? new Array(l2) : [], step, i2 = 0;
      var iterator = iter(iterable);
      while (true) {
        if (i2 === l2) return array;
        step = iterator.next();
        if (step.done) {
          if (i2 !== n2) array.length = i2;
          return array;
        }
        array[i2++] = step.value;
      }
...
var require_chain = __commonJS({
  "node_modules/obliterator/chain.js"(exports2, module2) {
    var Iterator = require_iterator();
    var iter = require_iter();
    module2.exports = function chain() {
...
var require_graphology_cjs = __commonJS({
  "node_modules/graphology/dist/graphology.cjs.js"(exports2, module2) {
    "use strict";
    ...
    function assignPolyfill() {
      ...
      for (var i2 = 1, l2 = arguments.length; i2 < l2; i2++) {
        ...
        for (var k2 in arguments[i2]) {
          target[k2] = arguments[i2][k2];
        }
      }
      return target;
    }
    ...
    function validateHints(hints) {
      if (!isPlainObject3(hints)) return false;
    ...
    function attachNodeAttributesReplacer(Class, method, mode) {
      Class.prototype[method] = function(nodeOrEdge, nameOrEdge, add1) {
        ...
        if (!isPlainObject3(attributes)) throw new InvalidArgumentsGraphError("Graph.".concat(method, ": provided attributes are not a plain object."));
    ...
    function forEachSimple(breakable, object, callback, avoid) {
      ...
      for (var k2 in object) {
        if (k2 === avoid) continue;
        var edgeData = object[k2];
    ...
    function forEachForKeySimple(breakable, object, k2, callback) {
      var edgeData = object[k2];
      if (!edgeData) return;
    ...
    function forEachForKeyMulti(breakable, object, k2, callback) {
      var edgeData = object[k2];
      if (!edgeData) return;
      var shouldBreak = false;
      do {
    ...
    function createIteratorForKey(object, k2) {
      var edgeData = object[k2];
    ...
    function createEdgeArray(graph2, type) {
      ...
      if (type === "mixed" || type === graph2.type) {
        if (typeof Array.from === "function") return Array.from(graph2._edges.keys());
        return take__default["default"](graph2._edges.keys(), graph2._edges.size);
      }
      var size = type === "undirected" ? graph2.undirectedSize : graph2.directedSize;
      var list = new Array(size), mask = type === "undirected";
      var iterator = graph2._edges.values();
      var i2 = 0;
      var step, data;
    ...
    function createEdgeIterator(graph2, type) {
      if (graph2.size === 0) return Iterator__default["default"].empty();
      var shouldFilter = type !== "mixed" && type !== graph2.type;
      var mask = type === "undirected";
      var iterator = graph2._edges.values();
      return new Iterator__default["default"](function next() {
    ...
    function attachForEachEdge(Class, description) {
      ...
      Class.prototype[mapName] = function() {
        ...
        if (args.length === 0) {
          ...
          var i2 = 0;
          args.push(function(e2, ea, s2, t2, sa, ta, u2) {
    ...
    function attachForEachNeighbor(Class, description) {
      ...
      Class.prototype[mapName] = function(node, callback) {
        var result = [];
        this[forEachName](node, function(n2, a2) {
    ...
    function attachFindNeighbor(Class, description) {
      ...
      Class.prototype[everyName] = function(node, callback) {
        var found = this[findName](node, function(n2, a2) {
    ...
    function forEachAdjacency(breakable, assymetric, disconnectedNodes, graph2, callback) {
      var iterator = graph2._nodes.values();
    ...
    function validateSerializedNode(value) {
      if (!isPlainObject3(value)) throw new InvalidArgumentsGraphError('Graph.import: invalid serialized node. A serialized node should be a plain object with at least a "key" property.');
      if (!("key" in value)) throw new InvalidArgumentsGraphError("Graph.import: serialized node is missing its key.");
      if ("attributes" in value && (!isPlainObject3(value.attributes) || value.attributes === null)) throw new InvalidArgumentsGraphError("Graph.import: invalid attributes. Attributes should be a plain object, null or omitted.");
    }
    function validateSerializedEdge(value) {
      if (!isPlainObject3(value)) throw new InvalidArgumentsGraphError('Graph.import: invalid serialized edge. A serialized edge should be a plain object with at least a "source" & "target" property.');
    ...
    function _addNode(graph2, node, attributes) {
      if (attributes && !isPlainObject3(attributes)) throw new InvalidArgumentsGraphError('Graph.addNode: invalid attributes. Expecting an object but got "'.concat(attributes, '"'));
      node = "" + node;
      attributes = attributes || {};
      if (graph2._nodes.has(node)) throw new UsageGraphError('Graph.addNode: the "'.concat(node, '" node already exist in the graph.'));
    ...
    function addEdge(graph2, name, mustGenerateKey, undirected, edge, source, target, attributes) {
      if (!undirected && graph2.type === "undirected") throw new UsageGraphError("Graph.".concat(name, ": you cannot add a directed edge to an undirected graph. Use the #.addEdge or #.addUndirectedEdge instead."));
      if (undirected && graph2.type === "directed") throw new UsageGraphError("Graph.".concat(name, ": you cannot add an undirected edge to a directed graph. Use the #.addEdge or #.addDirectedEdge instead."));
      if (attributes && !isPlainObject3(attributes)) throw new InvalidArgumentsGraphError("Graph.".concat(name, ': invalid attributes. Expecting an object but got "').concat(attributes, '"'));
      source = "" + source;
      target = "" + target;
      attributes = attributes || {};
      if (!graph2.allowSelfLoops && source === target) throw new UsageGraphError("Graph.".concat(name, ': source & target are the same ("').concat(source, `"), thus creating a loop explicitly forbidden by this graph 'allowSelfLoops' option set to false.`));
      ...
      if (mustGenerateKey) {
        ...
        edge = "" + edge;
        if (graph2._edges.has(edge)) throw new UsageGraphError("Graph.".concat(name, ': the "').concat(edge, '" edge already exists in the graph.'));
      }
    ...
    function mergeEdge(graph2, name, mustGenerateKey, undirected, edge, source, target, attributes, asUpdater) {
      if (!undirected && graph2.type === "undirected") throw new UsageGraphError("Graph.".concat(name, ": you cannot merge/update a directed edge to an undirected graph. Use the #.mergeEdge/#.updateEdge or #.addUndirectedEdge instead."));
      if (undirected && graph2.type === "directed") throw new UsageGraphError("Graph.".concat(name, ": you cannot merge/update an undirected edge to a directed graph. Use the #.mergeEdge/#.updateEdge or #.addDirectedEdge instead."));
      if (attributes) {
        if (asUpdater) {
          ...
          if (!isPlainObject3(attributes)) throw new InvalidArgumentsGraphError("Graph.".concat(name, ': invalid attributes. Expecting an object but got "').concat(attributes, '"'));
        }
      }
      source = "" + source;
      target = "" + target;
    ...
    var Graph2 = /* @__PURE__ */ (function(_EventEmitter) {
      ...
      _proto.mergeNode = function mergeNode(node, attributes) {
        if (attributes && !isPlainObject3(attributes)) throw new InvalidArgumentsGraphError('Graph.mergeNode: invalid attributes. Expecting an object but got "'.concat(attributes, '"'));
        node = "" + node;
        attributes = attributes || {};
      ...
      _proto.replaceAttributes = function replaceAttributes(attributes) {
        if (!isPlainObject3(attributes)) throw new InvalidArgumentsGraphError("Graph.replaceAttributes: provided attributes are not a plain object.");
      ...
      _proto.mergeAttributes = function mergeAttributes(attributes) {
        if (!isPlainObject3(attributes)) throw new InvalidArgumentsGraphError("Graph.mergeAttributes: provided attributes are not a plain object.");
      ...
      _proto["import"] = function _import(data) {
        ...
        if (data instanceof Graph3) {
          data.forEachNode(function(n2, a2) {
            if (merge) _this3.mergeNode(n2, a2);
            else _this3.addNode(n2, a2);
          });
          data.forEachEdge(function(e2, a2, s2, t2, _sa, _ta, u2) {
            if (merge) {
              if (u2) _this3.mergeUndirectedEdgeWithKey(e2, s2, t2, a2);
              else _this3.mergeDirectedEdgeWithKey(e2, s2, t2, a2);
            } else {
              if (u2) _this3.addUndirectedEdgeWithKey(e2, s2, t2, a2);
              else _this3.addDirectedEdgeWithKey(e2, s2, t2, a2);
            }
          });
          return this;
        }
        if (!isPlainObject3(data)) throw new InvalidArgumentsGraphError("Graph.import: invalid argument. Expecting a serialized graph or, alternatively, a Graph instance.");
        if (data.attributes) {
          if (!isPlainObject3(data.attributes)) throw new InvalidArgumentsGraphError("Graph.import: invalid attributes. Expecting a plain object.");
          if (merge) this.mergeAttributes(data.attributes);
          else this.replaceAttributes(data.attributes);
        }
        var i2, l2, list, node, edge;
        if (data.nodes) {
          list = data.nodes;
      ...
      _proto.copy = function copy(options) {
        ...
        var graph2 = this.emptyCopy(options);
        var iterator = this._edges.values();
        var step, edgeData;
        while (step = iterator.next(), step.done !== true) {
          edgeData = step.value;
      ...
      _proto.inspect = function inspect2() {
        ...
        var dummy = {};
        for (var k2 in this) {
          if (this.hasOwnProperty(k2) && !EMITTER_PROPS.has(k2) && typeof this[k2] !== "function" && _typeof(k2) !== "symbol") dummy[k2] = this[k2];
        }
...
function initGraph(context) {
  storagePath = path2.join(context.globalStorageUri.fsPath, "graph.json");
  graph = new import_graphology.default({ type: "directed", allowSelfLoops: false, multi: false });
  loadFromDisk();
}
...
function addImportEdge(fromFile, toFile2) {
  const fromK = fileKey(fromFile);
  const toK = fileKey(toFile2);
  if (!graph.hasNode(fromK)) addFileNode(fromFile, "");
  if (!graph.hasNode(toK)) addFileNode(toFile2, "");
  const edgeKey = `${fromK}->imports->${toK}`;
  if (!graph.hasEdge(edgeKey)) {
    graph.addEdgeWithKey(edgeKey, fromK, toK, { type: "imports" });
  }
}
function addReferenceEdge(fromFile, toFile2, symbolName) {
  const fromK = fileKey(fromFile);
  const toK = fileKey(toFile2);
  if (!graph.hasNode(fromK)) addFileNode(fromFile, "");
  if (!graph.hasNode(toK)) addFileNode(toFile2, "");
...
function getDependencies(filePath) {
  ...
  graph.forEachOutEdge(key, (_edge, attr, _src, target) => {
    if (attr.type === "imports") {
      deps.push(path2.basename(target));
    }
  });
  return deps;
}
function getDependents(filePath) {
  ...
  return Array.from(dependentMap.entries()).map(([file, count]) => ({ file: path2.basename(file), count })).sort((a2, b2) => b2.count - a2.count);
}
...
function clearFileEdges(filePath) {
  ...
  edgesToDrop.forEach((e2) => graph.dropEdge(e2));
}
function saveToDisk() {
  try {
    ...
    fs2.writeFileSync(storagePath, JSON.stringify(data));
  } catch {
  }
}
function loadFromDisk() {
  try {
    if (fs2.existsSync(storagePath)) {
      const raw = fs2.readFileSync(storagePath, "utf-8");
...
var SYMBOL_KIND_MAP = {
  [vscode2.SymbolKind.Function]: "function",
  [vscode2.SymbolKind.Method]: "method",
  [vscode2.SymbolKind.Class]: "class",
  [vscode2.SymbolKind.Constructor]: "method"
};
...
async function indexWorkspace(progress) {
  ...
  for (const file of files) {
    try {
      const doc = await vscode2.workspace.openTextDocument(file);
      await indexFile(doc);
      indexed++;
      if (progress) {
        progress.report({
...
async function indexReferences(document, symbols) {
  const filePath = document.uri.fsPath;
  const exportedSymbols = symbols.filter((s2) => SYMBOL_KIND_MAP[s2.kind]).slice(0, 15);
...
// src/prompts.ts
var BRIEF_SYSTEM_PROMPT = `You are boop, a concise code context assistant embedded in a developer's editor. You produce short, scannable file briefs that help new developers understand code instantly.
...
var CHANGES_SYSTEM_PROMPT = `You are boop, a concise code impact analyst. You assess uncommitted changes and tell the developer what their change MEANS \u2014 not what it IS (they can see the diff).
...
function buildChangesPrompt(ctx) {
  const impactList = ctx.impactedFiles.length > 0 ? ctx.impactedFiles.map((f2) => `${f2.file} (${f2.count} refs)`).join(", ") : "none detected";
